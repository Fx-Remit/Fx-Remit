#!/usr/bin/env node
/**
 * Ops: cancel a stuck pending-* remittance and restore spendable.
 *
 * Usage (from packages/services):
 *   pnpm exec node --import tsx --env-file=../../apps/pwa/.env.local \
 *     src/evm/scripts/cancel-stuck-pending.ts <orderId>
 *
 * Stuck Instant Send claim (broadcasting-*):
 *   ... cancel-stuck-pending.ts <orderId> --attach 0x...
 *   ... cancel-stuck-pending.ts <orderId> --force-release   # only after chain check
 */
import { prisma } from '@fx-remit/database';
import {
  TransactionService,
  ProviderOrderStillLiveError,
} from '../../transactions/transaction.service.js';

async function main() {
  const orderIdRaw = process.argv[2];
  if (!orderIdRaw) {
    console.error(
      'Usage: cancel-stuck-pending.ts <orderId> [--attach 0x...] [--force-release]',
    );
    process.exit(1);
  }

  try {
    const attachIdx = process.argv.indexOf('--attach');
    const attachHash =
      attachIdx >= 0 ? process.argv[attachIdx + 1]?.trim() : undefined;
    const forceRelease = process.argv.includes('--force-release');

    const orderId = BigInt(orderIdRaw);
    const tx = await prisma.transaction.findFirst({
      where: { orderId, type: 'REMITTANCE' },
      orderBy: { createdAt: 'desc' },
    });

    if (!tx) {
      console.error(`No REMITTANCE found for orderId=${orderIdRaw}`);
      process.exit(1);
    }

    console.log('Found:', {
      id: tx.id,
      orderId: tx.orderId.toString(),
      status: tx.status,
      txHash: tx.txHash,
      externalId: tx.externalId,
      amountUsd: tx.amountUsd.toString(),
    });

    if (attachHash) {
      if (!TransactionService.isOnChainTxHash(attachHash)) {
        console.error('--attach requires a 0x 32-byte hash');
        process.exit(1);
      }
      const attached = await TransactionService.attachOnChainHash({
        userId: tx.userId,
        orderId,
        txHash: attachHash,
      });
      console.log('Attached:', {
        status: attached?.status,
        txHash: attached?.txHash,
        chainId: attached?.chainId,
      });
      return;
    }

    if (TransactionService.isBroadcastClaimHash(tx.txHash)) {
      const paycrestOrderId =
        TransactionService.paycrestOrderIdFromTxHash(tx.txHash);
      if (!paycrestOrderId) {
        console.error('Cannot parse paycrest id from broadcasting-* hash');
        process.exit(1);
      }
      if (!forceRelease) {
        console.error(
          'Refusing: broadcasting-* claim may already have an on-chain send.',
        );
        console.error(
          'Use --attach 0x... if you have the hash, or --force-release only after confirming no USDC left the wallet.',
        );
        process.exit(1);
      }
      const released = await TransactionService.releaseBroadcastClaim({
        userId: tx.userId,
        orderId,
        paycrestOrderId,
      });
      console.log('Force-released claim:', released);
    }

    const refreshed = await prisma.transaction.findFirst({
      where: { orderId, type: 'REMITTANCE' },
      orderBy: { createdAt: 'desc' },
    });
    if (!refreshed) {
      process.exit(1);
    }

    if (
      !refreshed.txHash.startsWith('pending-') &&
      !refreshed.txHash.startsWith('abandoned-')
    ) {
      console.error('Refusing: txHash is not a placeholder (may be on-chain).');
      process.exit(1);
    }

    const key =
      refreshed.externalId ??
      refreshed.txHash.replace(/^(pending|abandoned)-/, '');
    if (!key) {
      console.error('No externalId / placeholder key to cancel with.');
      process.exit(1);
    }

    const paycrestId = refreshed.txHash.startsWith('pending-')
      ? refreshed.txHash.slice('pending-'.length)
      : null;

    try {
      const result = await TransactionService.cancelAbandonedPending(key);
      console.log('Cancelled:', {
        status: result?.status,
        txHash: result?.txHash,
        externalId: result?.externalId,
      });
    } catch (err) {
      if (err instanceof ProviderOrderStillLiveError) {
        console.error('Blocked (Paycrest may still be fundable):', err.message);
        if (
          paycrestId &&
          !TransactionService.isAppLocalPendingKey(
            paycrestId,
            refreshed.externalId,
          )
        ) {
          const { PayoutService } = await import(
            '../../paycrest/payout.service.js'
          );
          const live = await PayoutService.getSettlementOrder(paycrestId);
          const validUntil = (
            live.order as
              | { providerAccount?: { validUntil?: string } }
              | undefined
          )?.providerAccount?.validUntil;
          console.error(
            'Paycrest status:',
            (live.order as { status?: string } | undefined)?.status,
          );
          console.error('validUntil:', validUntil ?? '(unknown)');
          console.error(
            'Re-run this script after validUntil (order should be expired).',
          );
        } else {
          console.error(
            'Wait until Paycrest order is expired/cancelled/failed, then re-run.',
          );
        }
        process.exit(2);
      }
      throw err;
    }
  } finally {
    await prisma.$disconnect();
  }
}

main().catch(async (err) => {
  console.error(err);
  await prisma.$disconnect().catch(() => undefined);
  process.exit(1);
});
