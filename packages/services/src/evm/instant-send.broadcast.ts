...
export async function broadcastSettlementTransfer(opts: {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    // Never release broadcasting-* after Privy was invoked — timeout/substring
    // "rejects" can still mean the transfer was submitted. Ops: --attach / --force-release.
    console.error(
      '[InstantSend] Privy failure after claim; leaving broadcasting-* held',
