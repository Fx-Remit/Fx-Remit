      ...
      <div className="w-full max-w-[430px] flex flex-col min-h-screen bg-[#f8fafd] relative">
          ...
          <div className="flex w-full items-center">
            {(['All', 'Completed', 'Pending', 'Failed'] as TransactionStatus[]).map((tab) => (
              <button
                ...
                className={`flex-1 flex flex-col items-center pb-3 px-1 relative transition-all ${
                  activeTab === tab ? 'text-[#1C1C1C] font-bold' : 'text-[#888888] font-medium'
                }`}
              >
                <span className="text-[14px] sm:text-[16px] capitalize whitespace-nowrap">
                  {tab === 'All' ? 'All' : tab}
                </span>
                {activeTab === tab && (
            ...
            {filteredTransactions.map((tx) => (
              ...
              >
                ...
                <div className="space-y-1.5 flex-1 min-w-0">
                  <p className="text-[#05050580] text-[16px] font-semibold leading-none">
                    {tx.type}
                  </p>
                  <h3 className="text-[#1C1C1C] text-[18px] font-medium leading-none">{tx.pair}</h3>
                  <p className="text-[#05050580] text-[16px] font-semibold leading-none pt-2">
                    {tx.date}
                  </p>
                  ...
                  <div className="flex items-center gap-1.5 pt-4">
                    {tx.status === 'completed' && (
                      <>
                        ...
                        <span className="text-[#27A227] text-[18px] font-semibold leading-none">
                          Completed
                        </span>
                      </>
                    )}
                    {tx.status === 'failed' && (
                      <>
                        ...
                        <span className="text-red-500 text-[18px] font-semibold leading-none">
                          Failed
                        </span>
                      </>
                    )}
                    {tx.status === 'pending' && (
                      <>
                        ...
                        <span className="text-yellow-600 text-[18px] font-semibold leading-none">
                          Pending
                        </span>
                      </>
                    )}
                ...
                <div className="text-right space-y-2">
                  <div className="space-y-1">
                    ...
                    <p className="text-[#1C1C1C] text-[18px] font-semibold leading-none">
                      {tx.sentAmount}
                      {tx.sentToken}
                    </p>
                  </div>
                  <div className="pt-4 space-y-1">
                    <p className="text-[#6D6D6D] text-[16px] font-normal leading-none">
                      They receive
                    </p>
                    <p className="text-[#1C1C1C] text-[18px] font-semibold leading-none font-inter">
                      {tx.receivedToken}
                      {tx.receivedAmount}
                    </p>
                  </div>
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full px-5 flex justify-center max-w-[430px]">
          ...
          >
            ...
            <Link href="/history" className="flex flex-col items-center gap-1 text-[#1C1C1C]">
              <FileText size={28} />
